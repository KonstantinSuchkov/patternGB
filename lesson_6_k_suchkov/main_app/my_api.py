...
class API:
    ...
    def post_create_user(self, request_params):
        ...
        new_user = self.main.add_user(kind, name)
        try:
            parent_id = request_params['your_parent']  # если форма из родительской страницы (регистрация ребенка)
            parent = self.main.get_user(parent_id)  # нужно привязать "ребенка" к родителю
            parent[3].add_child(new_user)
        except:
            pass
